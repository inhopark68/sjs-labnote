...
class _NotesPageState extends State<NotesPage> {
  ...
  Future<String?> _askPasswordForExport() async {
    ...
    if (a.length < 4) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('비밀번호는 4자 이상을 권장합니다.')),
      );
      return null;
    }
                              ...
                              onPressed: _isBusy
                                  ...
                                  : () async {
                                      try {
                                        ...
                                        if (!mounted) return;
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(content: Text('핀 변경 실패: $e')),
                                        );
                                      }
                                    },
                            ),
                                ...
                                : () {
                                    ...
                                    showDialog(
                                      ...
                                      builder: (_) => AlertDialog(
                                        ...
                                        content: Text(item.title.isEmpty ? '(제목 없음)' : item.title),
                                        actions: [
                                          TextButton(
                                          ...
                                          TextButton(
                                            onPressed: () async {
                                              try {
                                                await rootContext.read<NotesVM>().deleteNoteById(id);
                                                if (!mounted) return;
