...
/// encoded가 Delta JSON(List)면 복원, 아니면 plain text로 문서 생성
void decodeDocOrPlainText(quill.QuillController c, String? encodedOrText) {
  ...
  if (raw.isEmpty) {
    ...
    c.updateSelection(const TextSelection.collapsed(offset: 0), quill.ChangeSource.local);
    return;
  }
...
class _NoteDetailPageState extends State<NoteDetailPage> {
  ...
  bool get _imageInsertSupported => !kIsWeb; // (원하면 web도 지원하도록 별도 처리 가능)
  ...
  // =====================================================
  // Load / Save (A안: title/body에 Delta JSON 저장)
  // =====================================================
  ...
  void _onTitleChanged() {
    ...
    _enforceTitleOneLine(); // 원치 않으면 이 줄 삭제 가능
  ...
  String _plainTextForTitle(quill.QuillController c) {
    // Quill plain text에는 이미지 embed 자리 등에 특수문자가 들어갈 수 있어요.
  ...
  // 제목을 사실상 한 줄로 유지하고 싶을 때(원치 않으면 호출 제거)
  void _enforceTitleOneLine() {
    ...
    try {
      ...
      _titleQuill.updateSelection(
        TextSelection.collapsed(offset: min(fixed.length, fixed.length)),
    ...
    return Container(
      ...
      child: quill.QuillEditor(
        ...
        configurations: quill.QuillEditorConfigurations(
          ...
          // ✅ 이미지/비디오 등 embed 렌더링을 extensions에서 제공
  ...
  // =====================================================
  // Image insert (카메라/갤러리 + nMB 이하 자동압축 + embed)
  // =====================================================
  ...
  Future<File> _compressToTargetMb({
    ...
    final outPath = p.join(
      dir.path,
      'note_img_${DateTime.now().millisecondsSinceEpoch}.jpg',
    );
...
// =====================================================
// Dialog payload types + dialogs (기존 그대로 유지)
// =====================================================
